...
This project focuses on altering paper helicopters by changing their rotor length, high leg width, high leg height, and whether or not its leg is clipped. We aim to answer the following research questions:
...
We performed a full factorial design for this experiment. This design appeared to best answer our research questions since we are interested in determining the best combination of all four factors. We also want to make sure that we can estimate the main effects of each factor as well as the interaction effect specifically between rotor length and leg width. A fractional factorial design would either not allow us to calculate all effects or require tedious planning on how to optimally craft the design. We felt that creating the $2^4$ combinations of a full factorial design was not enough to warrant considering a fractional design, so we went for the full design. 

The 16 conditions of paper planes are as follows:
...
We performed 5 trials for each condition, making a data set of 80 total observations. The treatments were completely randomized. 
...
plane <- plane %>%
  mutate(time = distance/100,
         rotor = as.factor(rotor),
...
```
The data appears to be almost bimodal with a peak around 1.6 seconds and another around 1.9 seconds. The average time overall was `r round(mean(plane$time), 2)`. 
...
mod.threeway = lm(time ~ rotor * leg_len * leg_wid * clip, data = plane)
summary(mod.threeway)
```
...
anova(mod.main, mod.threeway, test='F')
```

```{r}
#two-way vs. three-way
anova(mod.twoway, mod.threeway, test='F')
```


## Question 1: Which factors seem to be the most important for making helicopters that fly longer?


## Question 2: Is there any evidence that the effect of rotor length differs by leg width?


## Question 3: What would you recommend as the ideal combination to make the helicopter fly long?
